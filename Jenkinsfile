pipeline {
    ...
    stages {
        ...
        stage('4. Deploy Observability Stack') {
            steps {
                echo "Deploying Prometheus and Grafana via Helm..."
                sh """
                aws ssm send-command \
                  --targets "Key=tag:Name,Values=WebServer-1" \
                  --document-name "AWS-RunShellScript" \
                  --parameters 'commands=[
                    "export KUBECONFIG=/home/ec2-user/.kube/config",
                    ...
                    "echo \\"Environment=KUBECONFIG=/home/ec2-user/.kube/config\\" | sudo tee -a /etc/systemd/system/grafana-forward.service",
                    "echo \\"ExecStart=/usr/local/bin/kubectl port-forward --address 0.0.0.0 -n monitoring svc/prometheus-stack-grafana 30006:80\\" | sudo tee -a /etc/systemd/system/grafana-forward.service",
                    "echo \\"Restart=always\\" | sudo tee -a /etc/systemd/system/grafana-forward.service",
                    "echo \\"User=ec2-user\\" | sudo tee -a /etc/systemd/system/grafana-forward.service",
